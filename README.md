...
---
**遗留问题：**  
18. detail 视图按住 ctrl 点击首页，不能重定向到首页。  
19. 在用户页面有该用户最近创建的主题列表，在点击某主题最后回复信息时，跳转到该主题页面，但不能定位到最后一条回复处，并出现报错。  
20. 在 vue 中使用 ueditor 时出现了问题，似乎使用 ueditor 所对应的路由必须是一级的，改成二级路由就会报错。
