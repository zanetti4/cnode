  ...
  <section class="recentto">
    <Card :bordered="false" :dis-hover="true">
      <Breadcrumb slot="title" class="collec-bread">
        ...
        <BreadcrumbItem>{{getLoginname}} 参与的话题</BreadcrumbItem>
      </Breadcrumb>
      <Spin size="large" fix v-if="spinShow"></Spin>
      <CellGroup>
        <article v-for="item in curRecentReplies" :key="item.id">
      ...
      <Page :total="recentReplies.length" :current="curPage" :page-size="pageSize" @on-change="changePage" class-name="mainpage" />
    </Card>
  </section>
</template>
...
export default {
  ...
  async created(){
    ...
    this.user = data.data;
    this.recentReplies = this.user.recent_replies;
    
    let maxPage = Math.ceil(this.recentReplies.length/this.pageSize);
    ...
    this.curRecentReplies = this.recentReplies.slice(start, start + this.pageSize);
    //不能拿到用户创建的所有主题
    this.getDetails();
  },
  methods: {
    ...
    async getDetails(){
      ...
      for(let i = 0; i < this.curRecentReplies.length; i++){
        let {data} = await this.$api.getDetail(this.curRecentReplies[i].id);
